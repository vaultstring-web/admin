...
export function ReverseTransactionDialog({
  ...
  const alreadyReversed =
    String(transaction?.status ?? '').toLowerCase() === 'reversed';
      ...
      <DialogContent className="sm:max-w-106.25 border-none shadow-2xl bg-white dark:bg-slate-950">
        <DialogHeader>
          <div className="flex items-center gap-3 mb-1 text-destructive">
            ...
            <DialogTitle className="text-xl">Reverse Transaction</DialogTitle>
          </div>
          <DialogDescription className="text-slate-500 dark:text-slate-400">
            This will attempt to reverse transaction{' '}
            <span className="font-mono text-slate-900 dark:text-slate-200">
              #{transaction?.id}
            </span>
            . Funds will be moved back to the sender. This action is audited.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 py-4 border-y border-slate-100 dark:border-slate-800 my-2">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">
              {transaction?.currency === 'MWK' ? 'MWK' : '¥'}{' '}
              {transaction?.rawAmount.toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3 py-2">
          ...
          <Textarea
            ...
            placeholder="e.g., Customer entered wrong recipient; reversing per support case #1234"
          ...
          <p className="text-[11px] text-slate-400 italic">
            * Reversals are restricted by transaction state. If it’s already
            settled externally, use disputes/settlement reconciliation.
          </p>
        </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button
            variant="destructive"
            onClick={onReverse}
            disabled={alreadyReversed || !reason.trim() || isReversing}
